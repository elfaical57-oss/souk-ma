        ...
        <div className="container py-3">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-1.5 text-sm text-gray-500 min-w-0">
              <span className="hover:text-primary cursor-pointer shrink-0" onClick={() => router.push("/")}>
          ...
          {/* Mobile search */}
          <form onSubmit={(e) => { e.preventDefault(); go({ search: localSearch }); }} className="md:hidden mt-2.5 flex items-center border border-gray-200 rounded-xl overflow-hidden bg-gray-50">
            <Search className="w-4 h-4 text-gray-400 mx-3 shrink-0" />
            <input value={localSearch} onChange={(e) => setLocalSearch(e.target.value)}
              placeholder={lang === "ar" ? "ابحث في المنتجات..." : "Rechercher dans les produits..."}
              className="flex-1 py-2.5 text-sm text-gray-700 outline-none bg-transparent" />
            {localSearch && <button type="button" onClick={() => { setLocalSearch(""); go({ search: "" }); }} className="px-3"><X className="w-3.5 h-3.5 text-gray-400" /></button>}
            <button type="submit" className="bg-primary h-full px-4 py-2.5 flex items-center"><Search className="w-4 h-4 text-white" /></button>
          </form>
        </div>
      </div>
