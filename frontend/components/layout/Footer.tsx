...
const SOCIAL = [
  {
    name: "Facebook",
    href: "https://www.facebook.com/share/1D5LBB6cK1/?mibextid=wwXIfr",
    icon: (
  ...
  {
    name: "Instagram",
    href: "https://www.instagram.com/jemlamaroc_officiel?igsh=N2RqMXBlZGtkbWI0&utm_source=qr",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center gap-2 mt-5">
            {SOCIAL.map(s => (
              <a
                ...
                aria-label={s.name}
                className="w-9 h-9 bg-white/10 hover:bg-white/20 rounded-xl flex items-center justify-center transition-colors text-blue-200 hover:text-white"
              >
                {s.icon}
              </a>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-2.5">
            {LINKS.acheteurs.map(l => (
              <li key={l.href}>
                <Link href={l.href} className="text-sm text-blue-300 hover:text-white transition-colors">
                  {l.label}
                </Link>
              </li>
            ))}
